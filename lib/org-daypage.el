...
(setq daypage-template-file (expand-file-name "~/.daypage-template.org"))
...
(defun find-daypage (&optional date)
  ...
  ;; Modified from the original to insert template
  (when (and (eq 0 (buffer-size))
             (file-exists-p daypage-template-file))
    ;; Insert an initial template for the page
    (insert-file-contents daypage-template-file)
    (save-buffer)))
